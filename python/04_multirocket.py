...
def load_raw_data() -> tuple[pd.DataFrame, list[str]]:
    ...
    if not frames:
        raise RuntimeError("No raw files could be read successfully. See multirocket_read_log.txt for details.")
...
def run_within_multirocket(all_dt: pd.DataFrame, x_cols: list[str], y_cols: list[str], z_cols: list[str]) -> None:
    ...
    for ds in sorted(within_ready):
        ...
        save_text_summary(ds_stat / "multirocket_summary.txt", lines)
    ...
    if metrics_all:
        ...
        save_summary_metric_plot(summary_dt, "analysis_dataset", plots_dir / "within_summary_metrics.png", "Within MultiRocket | Summary metrics")
...
def run_inter_multirocket(all_dt: pd.DataFrame, x_cols: list[str], y_cols: list[str], z_cols: list[str]) -> None:
    ...
    for family, (ds1, ds2) in INTER_FAMILIES.items():
        for train_ds, test_ds in [(ds1, ds2), (ds2, ds1)]:
            ...
            lines.extend(pd.DataFrame(fit["overall_metrics"]).to_string(index=False).splitlines())
            save_text_summary(pair_stat / "multirocket_summary.txt", lines)
    ...
    if metrics_all:
        ...
        save_summary_metric_plot(summary_dt, "pair_id", plots_dir / "inter_summary_metrics.png", "Inter MultiRocket | Summary metrics")
...
def run_cross_multirocket(all_dt: pd.DataFrame, x_cols: list[str], y_cols: list[str], z_cols: list[str]) -> None:
    ...
    for test_ds in all_datasets:
        ...
        lines.extend(pd.DataFrame(fit["overall_metrics"]).to_string(index=False).splitlines())
        save_text_summary(out_stat / "multirocket_summary.txt", lines)
    ...
    if metrics_all:
        ...
        save_summary_metric_plot(summary_dt, "test_dataset", plots_dir / "cross_summary_metrics.png", "Cross MultiRocket | Summary metrics")
...
def run_pairwise_multirocket(all_dt: pd.DataFrame, x_cols: list[str], y_cols: list[str], z_cols: list[str]) -> None:
    ...
    if metrics_all:
        ...
        save_summary_metric_plot(summary_dt, "pair_id", plots_dir / "pairwise_summary_metrics.png", "Pairwise MultiRocket | Summary metrics")
...
def main() -> int:
    try:
        ...
        if MultiRocketClassifier is None:
            raise ImportError(
                "aeon is not available. Install aeon before running multirocket_model_pairwise.py."
            ) from MULTIROCKET_IMPORT_ERROR
        ...
        for _, root, _ in enabled_modes:
            make_clean_dir(root)
        save_text_summary(OUT_ROOT / "multirocket_read_log.txt", read_log)
        save_text_summary(
            OUT_ROOT / "multirocket_run_config.txt",
            [
        ...
        counts_dt.to_csv(OUT_ROOT / "multirocket_counts_by_dataset_species_behavior.csv", index=False)
        log(f"[OK] CSV saved: {OUT_ROOT / 'multirocket_counts_by_dataset_species_behavior.csv'}")
        ...
        lookup_dt.to_csv(OUT_ROOT / "multirocket_analysis_dataset_lookup.csv", index=False)
        log(f"[OK] CSV saved: {OUT_ROOT / 'multirocket_analysis_dataset_lookup.csv'}")

        save_duplicate_report(meta_dt, OUT_ROOT / "multirocket_duplicate_window_report.csv")
        ...
        if not unmatched.empty:
            unmatched[["species", "dataset_group", "dataset_id", "dataset_folder"]].drop_duplicates().to_csv(
                OUT_ROOT / "multirocket_unmatched_dataset_keys.csv", index=False
            )
            raise RuntimeError("Some rows could not be mapped to analysis_dataset. Check multirocket_unmatched_dataset_keys.csv")
