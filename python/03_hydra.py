...
def load_raw_data() -> tuple[pd.DataFrame, list[str]]:
    ...
    if not frames:
        raise RuntimeError("No raw files could be read successfully. See hydra_read_log.txt for details.")
...
def clean_raw_data(df: pd.DataFrame) -> tuple[pd.DataFrame, list[str], list[str], list[str]]:
    ...
    df = df[df["behavior"].isin(KEEP_BEHAVIORS)]
    ...
    df = df.dropna(subset=x_cols + y_cols + z_cols)
...
def run_global_hydra(all_dt: pd.DataFrame, x_cols: list[str], y_cols: list[str], z_cols: list[str]) -> None:
    ...
    lines = [
        ...
        f"Hydra n_kernels: {HYDRA_N_KERNELS}",
        f"Hydra n_groups: {HYDRA_N_GROUPS}",
    ...
    save_text_summary(stats_dir / "global_summary.txt", lines)
...
def run_within_hydra(all_dt: pd.DataFrame, x_cols: list[str], y_cols: list[str], z_cols: list[str]) -> None:
    ...
    for ds in sorted(within_ready):
        ...
        save_text_summary(ds_stat / "hydra_summary.txt", lines)
    ...
    if metrics_all:
        ...
        save_summary_metric_plot(summary_dt, "analysis_dataset", plots_dir / "within_summary_metrics.png", "Within HYDRA | Summary metrics")
...
def run_inter_hydra(all_dt: pd.DataFrame, x_cols: list[str], y_cols: list[str], z_cols: list[str]) -> None:
    ...
    for family, (ds1, ds2) in INTER_FAMILIES.items():
        for train_ds, test_ds in [(ds1, ds2), (ds2, ds1)]:
            ...
            lines.extend(pd.DataFrame(fit["overall_metrics"]).to_string(index=False).splitlines())
            save_text_summary(pair_stat / "hydra_summary.txt", lines)
    ...
    if metrics_all:
        ...
        save_summary_metric_plot(summary_dt, "pair_id", plots_dir / "inter_summary_metrics.png", "Inter HYDRA | Summary metrics")
...
def run_cross_hydra(all_dt: pd.DataFrame, x_cols: list[str], y_cols: list[str], z_cols: list[str]) -> None:
    ...
    for test_ds in all_datasets:
        ...
        lines.extend(pd.DataFrame(fit["overall_metrics"]).to_string(index=False).splitlines())
        save_text_summary(out_stat / "hydra_summary.txt", lines)
    ...
    if metrics_all:
        ...
        save_summary_metric_plot(summary_dt, "test_dataset", plots_dir / "cross_summary_metrics.png", "Cross HYDRA | Summary metrics")
...
def run_pairwise_hydra(all_dt: pd.DataFrame, x_cols: list[str], y_cols: list[str], z_cols: list[str]) -> None:
    ...
    if metrics_all:
        ...
        save_summary_metric_plot(summary_dt, "pair_id", plots_dir / "pairwise_summary_metrics.png", "Pairwise HYDRA | Summary metrics")
...
def main() -> int:
    try:
        ...
        if HydraClassifier is None:
            raise ImportError(
                "aeon is not available. Install aeon before running hydra_model_pairwise.py."
            ) from HYDRA_IMPORT_ERROR
        ...
        for _, root, _ in enabled_modes:
            make_clean_dir(root)
        save_text_summary(OUT_ROOT / "hydra_read_log.txt", read_log)
        save_text_summary(
            OUT_ROOT / "hydra_run_config.txt",
            [
        ...
        counts_dt.to_csv(OUT_ROOT / "hydra_counts_by_dataset_species_behavior.csv", index=False)
        log(f"[OK] CSV saved: {OUT_ROOT / 'hydra_counts_by_dataset_species_behavior.csv'}")
        ...
        lookup_dt.to_csv(OUT_ROOT / "hydra_analysis_dataset_lookup.csv", index=False)
        log(f"[OK] CSV saved: {OUT_ROOT / 'hydra_analysis_dataset_lookup.csv'}")

        save_duplicate_report(meta_dt, OUT_ROOT / "hydra_duplicate_window_report.csv")

        del counts_dt, lookup_dt, meta_dt
        gc.collect()

        unmatched = all_dt[all_dt["analysis_dataset"] == "UNMAPPED"]
        if not unmatched.empty:
            unmatched[["species", "dataset_group", "dataset_id", "dataset_folder"]].drop_duplicates().to_csv(
                OUT_ROOT / "hydra_unmatched_dataset_keys.csv", index=False
            )
            raise RuntimeError("Some rows could not be mapped to analysis_dataset. Check hydra_unmatched_dataset_keys.csv")
