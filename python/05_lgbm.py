...
FEATURE_ROOT = BASE / "Output" / "Pre" / "features_by_dataset"
OUT_ROOT = BASE / "Output" / "LGBM"

GLOBAL_ROOT = OUT_ROOT / "Global_LGBM"
WITHIN_ROOT = OUT_ROOT / "Within_LGBM"
INTER_ROOT = OUT_ROOT / "Inter_LGBM"
CROSS_ROOT = OUT_ROOT / "Cross_LGBM"
PAIRWISE_ROOT = OUT_ROOT / "Pairwise_LGBM"
...
def get_mtry() -> int:
    if MTRY_VALUE is not None:
        return int(MTRY_VALUE)
    return max(2, int(math.floor(math.sqrt(len(FEATURE_COLS)))))


def get_colsample_bytree(mtry_now: int) -> float:
    return min(1.0, max(mtry_now / len(FEATURE_COLS), 1.0 / len(FEATURE_COLS)))
...
def metric_from_confusion(cm: pd.DataFrame) -> dict[str, float]:
    ...
    recall = recall.replace([np.inf, -np.inf], np.nan)
    precision = precision.replace([np.inf, -np.inf], np.nan)
...
def add_analysis_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["dataset_id", "dataset_group", "species", "name", "subject_key", "source_file", "behavior"]:
...
def fit_lgbm_model(train_df: pd.DataFrame, test_df: pd.DataFrame) -> dict[str, object]:
    ...
    mtry_now = get_mtry()
    colsample_now = get_colsample_bytree(mtry_now)
...
def write_basic_outputs(
    ...
    pd.DataFrame(train_balance).to_csv(stat_dir / "train_balance.csv", index=False)
    pd.DataFrame(test_balance).to_csv(stat_dir / "test_balance.csv", index=False)
    pd.DataFrame(fit["cm_dt"]).to_csv(stat_dir / "confusion_matrix.csv", index=False)
    pd.DataFrame(fit["overall_metrics"]).to_csv(stat_dir / "metrics_overall.csv", index=False)
    pd.DataFrame(fit["behavior_metrics"]).to_csv(stat_dir / "metrics_by_behavior.csv", index=False)
    pd.DataFrame(fit["varimp_dt"]).to_csv(stat_dir / "variable_importance.csv", index=False)

    save_cm_plot(pd.DataFrame(fit["cm_dt"]), plot_dir / "confusion_matrix.png", f"{title_prefix} | Confusion matrix")
    save_varimp_plot(pd.DataFrame(fit["varimp_dt"]), plot_dir / "variable_importance_top15.png", f"{title_prefix} | Top 15 variable importance")
    ...
    save_text_summary(stat_dir / "lgbm_summary.txt", summary_lines)


def run_global_lgbm(all_dt: pd.DataFrame) -> None:
    log("\n================ GLOBAL LGBM ================")
    ...
    fit = fit_lgbm_model(train_bal, test_dt)
    ...
    global_metrics_by_dataset.to_csv(stats_dir / "metrics_by_analysis_dataset.csv", index=False)
    save_summary_metric_plot(global_metrics_by_dataset, "analysis_dataset", plots_dir / "dataset_metrics.png", "Global LGBM | Metrics by dataset")
    ...
    summary_lines = [
        ...
        f"mtry: {fit['mtry_now']}",
        f"colsample_bytree: {fit['colsample_bytree']:.4f}",
        f"min_leaf: {MIN_LEAF}",
...
def run_within_lgbm(all_dt: pd.DataFrame) -> None:
    log("\n================ WITHIN LGBM ================")
    ...
    if metrics_all:
        ...
        metrics_all_df.to_csv(stats_dir / "within_metrics_all.csv", index=False)
        behavior_all_df.to_csv(stats_dir / "within_behavior_metrics_all.csv", index=False)
        varimp_all_df.to_csv(stats_dir / "within_variable_importance_all.csv", index=False)
        save_summary_metric_plot(metrics_all_df, "analysis_dataset", plots_dir / "within_summary_metrics.png", "Within LGBM | Accuracy and macro F1")

        summary_lines = [
            "Mode: Within_LGBM summary",
        ...
        save_text_summary(stats_dir / "within_summary.txt", summary_lines)


def run_inter_lgbm(all_dt: pd.DataFrame) -> None:
    log("\n================ INTER LGBM ================")
    ...
    for family, (ds1, ds2) in INTER_FAMILIES.items():
        for train_ds, test_ds in [(ds1, ds2), (ds2, ds1)]:
            ...
            write_basic_outputs(
                ...
                plot_dir=pair_plot,
                stat_dir=pair_stat,
                title_prefix=f"Inter LGBM | {pair_id}",
                train_balance=train_balance,
                test_balance=test_balance,
                summary_lines=summary_lines,
            )
    ...
    if metrics_all:
        ...
        metrics_all_df.to_csv(stats_dir / "inter_metrics_all.csv", index=False)
        behavior_all_df.to_csv(stats_dir / "inter_behavior_metrics_all.csv", index=False)
        varimp_all_df.to_csv(stats_dir / "inter_variable_importance_all.csv", index=False)
        save_summary_metric_plot(metrics_all_df, "pair_id", plots_dir / "inter_summary_metrics.png", "Inter LGBM | Accuracy and macro F1")

        summary_lines = [
            "Mode: Inter_LGBM summary",
            "Pairs completed: " + ", ".join(sorted(metrics_all_df["pair_id"].tolist())),
            "Skipped pairs: none" if not skip_log else "Skipped pairs:\n  - " + "\n  - ".join(skip_log),
        ]
        save_text_summary(stats_dir / "inter_summary.txt", summary_lines)


def run_cross_lgbm(all_dt: pd.DataFrame) -> None:
    log("\n================ CROSS LGBM ================")
    ...
    for test_ds in all_datasets:
        ...
        write_basic_outputs(
            ...
            plot_dir=out_plot,
            stat_dir=out_stat,
            title_prefix=f"Cross LGBM | test={test_ds}",
            train_balance=train_balance,
            test_balance=test_balance,
            summary_lines=summary_lines,
        )
    ...
    if metrics_all:
        ...
        metrics_all_df.to_csv(stats_dir / "cross_metrics_all.csv", index=False)
        behavior_all_df.to_csv(stats_dir / "cross_behavior_metrics_all.csv", index=False)
        varimp_all_df.to_csv(stats_dir / "cross_variable_importance_all.csv", index=False)
        save_summary_metric_plot(metrics_all_df, "test_dataset", plots_dir / "cross_summary_metrics.png", "Cross LGBM | Accuracy and macro F1")

        summary_lines = [
            "Mode: Cross_LGBM summary",
            "Datasets completed: " + ", ".join(sorted(metrics_all_df["test_dataset"].tolist())),
            "Skipped datasets: none" if not skip_log else "Skipped datasets:\n  - " + "\n  - ".join(skip_log),
        ]
        save_text_summary(stats_dir / "cross_summary.txt", summary_lines)




def run_pairwise_lgbm(all_dt: pd.DataFrame) -> None:
    log("\n================ PAIRWISE LGBM ================")
    ...
    for train_ds in all_datasets:
        for test_ds in all_datasets:
            ...
            write_basic_outputs(
                ...
                plot_dir=pair_plot,
                stat_dir=pair_stat,
                title_prefix=f"Pairwise LGBM | {pair_id}",
                train_balance=train_balance,
                test_balance=test_balance,
                summary_lines=summary_lines,
            )
    ...
    if metrics_all:
        ...
        metrics_all_df.to_csv(stats_dir / "pairwise_metrics_all.csv", index=False)
        behavior_all_df.to_csv(stats_dir / "pairwise_behavior_metrics_all.csv", index=False)
        varimp_all_df.to_csv(stats_dir / "pairwise_variable_importance_all.csv", index=False)
        save_summary_metric_plot(metrics_all_df, "pair_id", plots_dir / "pairwise_summary_metrics.png", "Pairwise LGBM | Accuracy and macro F1")

        summary_lines = [
            "Mode: Pairwise_LGBM summary",
            "Pairs completed: " + ", ".join(sorted(metrics_all_df["pair_id"].tolist())),
            "Skipped pairs: none" if not skip_log else "Skipped pairs:\n  - " + "\n  - ".join(skip_log),
        ]
        save_text_summary(stats_dir / "pairwise_summary.txt", summary_lines)
...
def main() -> int:
    ...
    read_log_path = OUT_ROOT / "lgbm_read_log.txt"
    ...
    counts_dt.to_csv(OUT_ROOT / "lgbm_counts_by_dataset_species_behavior.csv", index=False)
    log(f"[OK] CSV saved: {OUT_ROOT / 'lgbm_counts_by_dataset_species_behavior.csv'}")
    ...
    lookup_dt.to_csv(OUT_ROOT / "lgbm_analysis_dataset_lookup.csv", index=False)
    log(f"[OK] CSV saved: {OUT_ROOT / 'lgbm_analysis_dataset_lookup.csv'}")

    save_duplicate_report(all_dt, OUT_ROOT / "lgbm_duplicate_window_report.csv")
    ...
    if not unmatched.empty:
        unmatched[["species", "dataset_group", "dataset_id", "dataset_folder"]].drop_duplicates().to_csv(
            OUT_ROOT / "lgbm_unmatched_dataset_keys.csv", index=False
        )
        raise RuntimeError("Some rows could not be mapped to analysis_dataset. Check lgbm_unmatched_dataset_keys.csv")

    run_global_lgbm(all_dt)
    run_within_lgbm(all_dt)
    run_inter_lgbm(all_dt)
    run_cross_lgbm(all_dt)
    run_pairwise_lgbm(all_dt)

    log("\n========================================")
    log("[DONE] LGBM pipeline finished.")
