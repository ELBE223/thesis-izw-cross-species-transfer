...
def load_raw_data() -> tuple[pd.DataFrame, list[str]]:
    ...
    if not frames:
        raise RuntimeError("No raw files could be read successfully. See cnn_read_log.txt for details.")
...
def fit_cnn_model(
    ...
    feature_names = [f"channel_{i}" for i in range(y_prob.shape[1])]
    del feature_names
...
def run_global_cnn(all_dt: pd.DataFrame, x_cols: list[str], y_cols: list[str], z_cols: list[str], device: torch.device) -> None:
    ...
    save_text_summary(stats_dir / "cnn_summary.txt", summary_lines)
...
def run_within_cnn(all_dt: pd.DataFrame, x_cols: list[str], y_cols: list[str], z_cols: list[str], device: torch.device) -> None:
    ...
    for ds in sorted(within_ready):
        ...
        summary_lines.extend(pd.DataFrame(fit["overall_metrics"]).to_string(index=False).splitlines())
        save_text_summary(ds_stat / "cnn_summary.txt", summary_lines)
    ...
    if metrics_all:
        ...
        save_summary_metric_plot(summary_dt, "analysis_dataset", plots_dir / "within_summary_metrics.png", "Within CNN | Summary metrics")
...
def run_inter_cnn(all_dt: pd.DataFrame, x_cols: list[str], y_cols: list[str], z_cols: list[str], device: torch.device) -> None:
    ...
    for family, (ds1, ds2) in INTER_FAMILIES.items():
        ...
        f"Validation mode: disabled (aligned to HYDRA/MultiRocket)",
            ...
            summary_lines.extend(pd.DataFrame(fit["overall_metrics"]).to_string(index=False).splitlines())
            save_text_summary(pair_stat / "cnn_summary.txt", summary_lines)

    if metrics_all:
        summary_dt = pd.concat(metrics_all, ignore_index=True).sort_values(["macro_f1", "accuracy"], ascending=[False, False])
        summary_dt.to_csv(stats_dir / "inter_summary_metrics.csv", index=False)
        save_summary_metric_plot(summary_dt, "pair_id", plots_dir / "inter_summary_metrics.png", "Inter CNN | Summary metrics")
...
def run_cross_cnn(all_dt: pd.DataFrame, x_cols: list[str], y_cols: list[str], z_cols: list[str], device: torch.device) -> None:
    ...
    for test_ds in all_datasets:
        ...
        summary_lines.extend(pd.DataFrame(fit["overall_metrics"]).to_string(index=False).splitlines())
        save_text_summary(out_stat / "cnn_summary.txt", summary_lines)

    if metrics_all:
        summary_dt = pd.concat(metrics_all, ignore_index=True).sort_values(["macro_f1", "accuracy"], ascending=[False, False])
        summary_dt.to_csv(stats_dir / "cross_summary_metrics.csv", index=False)
        save_summary_metric_plot(summary_dt, "test_dataset", plots_dir / "cross_summary_metrics.png", "Cross CNN | Summary metrics")
...
def run_pairwise_cnn(all_dt: pd.DataFrame, x_cols: list[str], y_cols: list[str], z_cols: list[str], device: torch.device) -> None:
    ...
    if metrics_all:
        ...
        save_summary_metric_plot(summary_dt, "pair_id", plots_dir / "pairwise_summary_metrics.png", "Pairwise CNN | Summary metrics")
...
def main() -> int:
    try:
        ...
        for root in [GLOBAL_ROOT, WITHIN_ROOT, INTER_ROOT, CROSS_ROOT, PAIRWISE_ROOT]:
            make_clean_dir(root)
        save_text_summary(OUT_ROOT / "cnn_read_log.txt", read_log)
        ...
        counts_dt.to_csv(OUT_ROOT / "cnn_counts_by_dataset_species_behavior.csv", index=False)
        log(f"[OK] CSV saved: {OUT_ROOT / 'cnn_counts_by_dataset_species_behavior.csv'}")
        ...
        lookup_dt.to_csv(OUT_ROOT / "cnn_analysis_dataset_lookup.csv", index=False)
        log(f"[OK] CSV saved: {OUT_ROOT / 'cnn_analysis_dataset_lookup.csv'}")

        save_duplicate_report(all_dt, OUT_ROOT / "cnn_duplicate_window_report.csv")
