...
from aeon.classification.deep_learning import InceptionTimeClassifier, IndividualInceptionClassifier # type: ignore
...
OUT_ROOT = BASE / "Output" / "InceptionTime"

GLOBAL_ROOT = OUT_ROOT / "Global_InceptionTime"
WITHIN_ROOT = OUT_ROOT / "Within_InceptionTime"
INTER_ROOT = OUT_ROOT / "Inter_InceptionTime"
CROSS_ROOT = OUT_ROOT / "Cross_InceptionTime"
PAIRWISE_ROOT = OUT_ROOT / "Pairwise_InceptionTime"
MODEL_TMP_ROOT = OUT_ROOT / "_model_cache"
...
def reset_tf_state(seed: int) -> None:
...
def get_device() -> str:
    ...
    if gpus:
        return ", ".join([f"GPU:{i}" for i, _ in enumerate(gpus)])
    cpus = tf.config.list_physical_devices("CPU")
    if cpus:
        return ", ".join([f"CPU:{i}" for i, _ in enumerate(cpus)])
    return "UNKNOWN"
...
def reset_model_cache_dir(path: Path) -> None:
    if path.exists():
        shutil.rmtree(path)
    path.mkdir(parents=True, exist_ok=True)
    log(f"[OK] Model cache reset: {path}")
...
def load_raw_data() -> tuple[pd.DataFrame, list[str]]:
    ...
    if not frames:
        raise RuntimeError("No raw files could be read successfully. See inceptiontime_read_log.txt for details.")
...
def build_history_df_from_model(model: object) -> pd.DataFrame:
    ...
    loss = list(history_dict.get("loss", []))
    val_loss = list(history_dict.get("val_loss", []))

    n_epochs = max(len(loss), len(val_loss))
    if n_epochs == 0:
        return pd.DataFrame(columns=["epoch", "train_loss", "val_loss"])
    ...
    for i in range(n_epochs):
        rows.append(
            {
                "epoch": i + 1,
                "train_loss": float(loss[i]) if i < len(loss) else np.nan,
                "val_loss": float(val_loss[i]) if i < len(val_loss) else np.nan,
            }
        )
    return pd.DataFrame(rows)


class SampleWeightedInceptionTimeClassifier(IndividualInceptionClassifier):
    def __init__(
        self,
        n_classifiers: int = 1,
        ...
        self.fit_sample_weight = fit_sample_weight
        ...
        if self.n_classifiers != 1:
            raise ValueError(
                "SampleWeightedInceptionTimeClassifier supports only n_classifiers=1. "
                "Use aeon InceptionTimeClassifier for ensembles."
            )
    ...
    def _fit(self, X, y):
        ...
        if self.fit_sample_weight is not None:
            fit_kwargs["sample_weight"] = np.asarray(self.fit_sample_weight, dtype=np.float32)

        self.history = self.training_model_.fit(X, y_onehot, **fit_kwargs)
...
def make_inception_optimizer() -> tf.keras.optimizers.Optimizer:
    adam_kwargs = {"learning_rate": LEARNING_RATE}
    try:
        return tf.keras.optimizers.Adam(weight_decay=WEIGHT_DECAY, **adam_kwargs)
    except TypeError:
        return tf.keras.optimizers.Adam(**adam_kwargs)


def get_inception_optimizer_note(optimizer: tf.keras.optimizers.Optimizer) -> str:
    has_weight_decay = hasattr(optimizer, "weight_decay") and getattr(optimizer, "weight_decay") is not None
    if has_weight_decay:
        return f"Adam(lr={LEARNING_RATE}, weight_decay={WEIGHT_DECAY})"
    return f"Adam(lr={LEARNING_RATE}, weight_decay=not_supported_in_local_tf)"
...
def fit_inceptiontime_model(
    ...
    if train_df.empty:
        raise ValueError("fit_inceptiontime_model received an empty training table.")
    if test_df.empty:
        raise ValueError("fit_inceptiontime_model received an empty test table.")
    ...
    y_train = train_main_df["behavior"].astype(str).to_numpy()
    y_test = test_df["behavior"].astype(str).to_numpy()
    ...
    train_sample_weight = train_main_df["behavior"].map(class_weights).to_numpy(dtype=np.float32)

    reset_tf_state(SEED)
    reset_model_cache_dir(MODEL_TMP_ROOT)

    optimizer = make_inception_optimizer()
    optimizer_note = get_inception_optimizer_note(optimizer)

    model = SampleWeightedInceptionTimeClassifier(
        n_classifiers=1,
        n_epochs=EPOCHS,
        batch_size=BATCH_SIZE,
        file_path=str(MODEL_TMP_ROOT) + "/",
        ...
        metrics="accuracy",
        optimizer=optimizer,
        fit_sample_weight=train_sample_weight,
    )

    log(
        "[INFO] InceptionTime setup | n_classifiers=1 | "
        f"optimizer={optimizer_note} | class_imbalance=sample_weight_from_compute_class_weights"
    )
    model.fit(X_train, y_train)

    pred_values = model.predict(X_test)
    prob_values = model.predict_proba(X_test)

    prob_df = pd.DataFrame(prob_values, columns=[str(x) for x in model.classes_])
    prob_df = prob_df.reindex(columns=KEEP_BEHAVIORS, fill_value=0.0)

    truth = pd.Series(y_test, index=test_df.index, dtype="object")
    pred = pd.Series(pd.Series(pred_values).astype(str).to_numpy(), index=test_df.index, dtype="object")
    ...
    history_df = build_history_df_from_model(model)

    return {
        "model": model,
        "pred": pred,
        "truth": truth,
        "prob": prob_df.to_numpy(dtype=np.float32),
        "cm_dt": cm_dt,
        ...
        "imbalance_strategy": "sample_weight_from_compute_class_weights",
    }
...
def build_family_lookup_map(df: pd.DataFrame) -> dict[str, str]:
    lookup_dt = (
        df[["analysis_dataset", "analysis_family"]]
        .drop_duplicates()
        .dropna(subset=["analysis_dataset"])
    )
    return dict(zip(lookup_dt["analysis_dataset"], lookup_dt["analysis_family"]))
...
def get_enabled_modes() -> list[tuple[str, Path, Callable[..., None]]]:
    ...
    if RUN_GLOBAL:
        modes.append(("Global_InceptionTime", GLOBAL_ROOT, run_global_inceptiontime))
    if RUN_WITHIN:
        modes.append(("Within_InceptionTime", WITHIN_ROOT, run_within_inceptiontime))
    if RUN_INTER:
        modes.append(("Inter_InceptionTime", INTER_ROOT, run_inter_inceptiontime))
    if RUN_CROSS:
        modes.append(("Cross_InceptionTime", CROSS_ROOT, run_cross_inceptiontime))
    if RUN_PAIRWISE:
        modes.append(("Pairwise_InceptionTime", PAIRWISE_ROOT, run_pairwise_inceptiontime))
...
# Modes
# =========================================================
def run_global_inceptiontime(all_dt: pd.DataFrame, x_cols: list[str], y_cols: list[str], z_cols: list[str], device: str) -> None:
    log("\n================ GLOBAL InceptionTime ================")
    plots_dir, stats_dir = prepare_mode_dirs(GLOBAL_ROOT)

    family_lookup = build_family_lookup_map(all_dt)
    ...
    fit = fit_inceptiontime_model(train_bal, test_dt, x_cols, y_cols, z_cols, device)
    ...
    for ds, sub in test_dt.groupby("analysis_dataset", dropna=False):
        ...
        row["analysis_family"] = family_lookup.get(ds, "")
    ...
    save_summary_metric_plot(metrics_by_dataset, "analysis_dataset", plots_dir / "dataset_metrics.png", "Global InceptionTime | Metrics by dataset")

    summary_lines = [
        "Mode: Global_InceptionTime",
        f"Device: {device}",
        f"Train rows after balancing: {len(train_bal)}",
        f"Train rows used for fitting: {fit['train_main_n']}",
        f"Validation rows: {fit['val_n']}",
        "Validation mode: disabled (aligned to HYDRA/MultiRocket)",
        ...
        f"Optimizer: {fit['optimizer_note']}",
            f"Class imbalance: {fit['imbalance_strategy']}",
        "",
        "Overall metrics:",
    ]
    ...
    save_text_summary(stats_dir / "inceptiontime_summary.txt", summary_lines)

def run_within_inceptiontime(all_dt: pd.DataFrame, x_cols: list[str], y_cols: list[str], z_cols: list[str], device: str) -> None:
    log("\n================ WITHIN InceptionTime ================")
    ...
    for ds in sorted(within_ready):
        ...
        split_dt, split_notes = make_holdout_split(ds_dt, "analysis_dataset", TEST_FRAC, SEED)
        if split_dt is None:
        ...
        write_basic_outputs(
            ...
            ds_plot,
            ds_stat,
            f"Within InceptionTime | {ds}",
        )

        summary_lines = [
            "Mode: Within_InceptionTime",
            f"Dataset: {ds}",
            f"Family: {family_lookup.get(ds, '')}",
            f"Device: {device}",
            f"Train rows after balancing: {len(train_bal)}",
            f"Train rows used for fitting: {fit['train_main_n']}",
            f"Validation rows: {fit['val_n']}",
        ...
        if split_notes:
            summary_lines.append("")
            summary_lines.append("Split notes:")
            summary_lines.extend(split_notes)
        save_text_summary(ds_stat / "inceptiontime_summary.txt", summary_lines)
    ...
    if metrics_all:
        ...
        save_summary_metric_plot(summary_dt, "analysis_dataset", plots_dir / "within_summary_metrics.png", "Within InceptionTime | Summary metrics")

        behavior_all_df = pd.concat(behavior_all, ignore_index=True)
        behavior_all_df.to_csv(stats_dir / "within_behavior_metrics_all.csv", index=False)

        lines = ["Mode: Within_InceptionTime", "", "Summary metrics:"]
...
def run_inter_inceptiontime(all_dt: pd.DataFrame, x_cols: list[str], y_cols: list[str], z_cols: list[str], device: str) -> None:
    log("\n================ INTER InceptionTime ================")
    ...
    for family, (ds1, ds2) in INTER_FAMILIES.items():
        for train_ds, test_ds in [(ds1, ds2), (ds2, ds1)]:
            ...
            metrics_dt["train_family"] = family
            metrics_dt["test_family"] = family
            metrics_all.append(metrics_dt)
            ...
            behavior_dt["train_family"] = family
            behavior_dt["test_family"] = family
            behavior_all.append(behavior_dt)
            ...
            write_basic_outputs(
                ...
                pair_plot,
                pair_stat,
                f"Inter InceptionTime | {pair_id}",
            )

            summary_lines = [
                "Mode: Inter_InceptionTime",
                f"Family: {family}",
            ...
            summary_lines.extend(pd.DataFrame(fit["overall_metrics"]).to_string(index=False).splitlines())
            save_text_summary(pair_stat / "inceptiontime_summary.txt", summary_lines)

    if metrics_all:
        summary_dt = pd.concat(metrics_all, ignore_index=True).sort_values(["macro_f1", "accuracy"], ascending=[False, False])
        summary_dt.to_csv(stats_dir / "inter_summary_metrics.csv", index=False)
        save_summary_metric_plot(summary_dt, "pair_id", plots_dir / "inter_summary_metrics.png", "Inter InceptionTime | Summary metrics")

        behavior_all_df = pd.concat(behavior_all, ignore_index=True)
        behavior_all_df.to_csv(stats_dir / "inter_behavior_metrics_all.csv", index=False)

        lines = ["Mode: Inter_InceptionTime", "", "Summary metrics:"]
        lines.extend(summary_dt.to_string(index=False).splitlines())
        if skip_log:
            lines.append("")
            lines.append("Skip log:")
...
def run_cross_inceptiontime(all_dt: pd.DataFrame, x_cols: list[str], y_cols: list[str], z_cols: list[str], device: str) -> None:
    log("\n================ CROSS InceptionTime ================")
    ...
    for test_ds in all_datasets:
        ...
        write_basic_outputs(
            ...
            out_plot,
            out_stat,
            f"Cross InceptionTime | {test_ds}",
        )

        summary_lines = [
            "Mode: Cross_InceptionTime",
            f"Test dataset: {test_ds}",
        ...
        summary_lines.extend(pd.DataFrame(fit["overall_metrics"]).to_string(index=False).splitlines())
        save_text_summary(out_stat / "inceptiontime_summary.txt", summary_lines)

    if metrics_all:
        summary_dt = pd.concat(metrics_all, ignore_index=True).sort_values(["macro_f1", "accuracy"], ascending=[False, False])
        summary_dt.to_csv(stats_dir / "cross_summary_metrics.csv", index=False)
        save_summary_metric_plot(summary_dt, "test_dataset", plots_dir / "cross_summary_metrics.png", "Cross InceptionTime | Summary metrics")

        behavior_all_df = pd.concat(behavior_all, ignore_index=True)
        behavior_all_df.to_csv(stats_dir / "cross_behavior_metrics_all.csv", index=False)

        lines = ["Mode: Cross_InceptionTime", "", "Summary metrics:"]
        lines.extend(summary_dt.to_string(index=False).splitlines())
        if skip_log:
            lines.append("")
            lines.append("Skip log:")
...
def run_pairwise_inceptiontime(all_dt: pd.DataFrame, x_cols: list[str], y_cols: list[str], z_cols: list[str], device: str) -> None:
    log("\n================ PAIRWISE InceptionTime ================")
    ...
    for train_ds in all_datasets:
        for test_ds in all_datasets:
            ...
            write_basic_outputs(
                ...
                pair_plot,
                pair_stat,
                f"Pairwise InceptionTime | {pair_id}",
            )

            summary_lines = [
                "Mode: Pairwise_InceptionTime",
                f"Train dataset: {train_ds}",
                f"Test dataset: {test_ds}",
    ...
    if metrics_all:
        ...
        save_summary_metric_plot(summary_dt, "pair_id", plots_dir / "pairwise_summary_metrics.png", "Pairwise InceptionTime | Summary metrics")

        behavior_all_df = pd.concat(behavior_all, ignore_index=True)
        behavior_all_df.to_csv(stats_dir / "pairwise_behavior_metrics_all.csv", index=False)

        lines = ["Mode: Pairwise_InceptionTime", "", "Summary metrics:"]
        lines.extend(summary_dt.to_string(index=False).splitlines())
        if skip_log:
            lines.append("")
            lines.append("Skip log:")
...
def main() -> int:
    try:
        ...
        for root in [GLOBAL_ROOT, WITHIN_ROOT, INTER_ROOT, CROSS_ROOT, PAIRWISE_ROOT]:
            make_clean_dir(root)
        save_text_summary(OUT_ROOT / "inceptiontime_read_log.txt", read_log)
        ...
        counts_dt.to_csv(OUT_ROOT / "inceptiontime_counts_by_dataset_species_behavior.csv", index=False)
        log(f"[OK] CSV saved: {OUT_ROOT / 'inceptiontime_counts_by_dataset_species_behavior.csv'}")
        ...
        lookup_dt.to_csv(OUT_ROOT / "inceptiontime_analysis_dataset_lookup.csv", index=False)
        log(f"[OK] CSV saved: {OUT_ROOT / 'inceptiontime_analysis_dataset_lookup.csv'}")

        save_duplicate_report(all_dt, OUT_ROOT / "inceptiontime_duplicate_window_report.csv")
        ...
        log("\n========================================")
        if mode_failures:
            log("[ERROR] InceptionTime pipeline finished with errors.")
        ...
        log("[DONE] InceptionTime pipeline finished.")
        log(f"[DONE] Output root: {OUT_ROOT}")
        log(f"[DONE] remove_padded = {REMOVE_PADDED}")
        log("========================================")
        return 0
        ...
        log_exception("InceptionTime pipeline aborted", exc)
        return 1
