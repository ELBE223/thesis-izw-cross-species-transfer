...
def write_basic_outputs(
    ...
    save_text_summary(stat_dir / "rf_summary.txt", summary_lines)
...
def run_global_rf(all_dt: pd.DataFrame) -> None:
    ...
    save_summary_metric_plot(global_metrics_by_dataset, "analysis_dataset", plots_dir / "dataset_metrics.png", "Global RF | Metrics by dataset")
...
def run_within_rf(all_dt: pd.DataFrame) -> None:
    ...
    if metrics_all:
        ...
        save_summary_metric_plot(metrics_all_df, "analysis_dataset", plots_dir / "within_summary_metrics.png", "Within RF | Accuracy and macro F1")
...
def run_inter_rf(all_dt: pd.DataFrame) -> None:
    ...
    if metrics_all:
        ...
        save_summary_metric_plot(metrics_all_df, "pair_id", plots_dir / "inter_summary_metrics.png", "Inter RF | Accuracy and macro F1")
...
def run_cross_rf(all_dt: pd.DataFrame) -> None:
    ...
    if metrics_all:
        ...
        save_summary_metric_plot(metrics_all_df, "test_dataset", plots_dir / "cross_summary_metrics.png", "Cross RF | Accuracy and macro F1")
...
def run_pairwise_rf(all_dt: pd.DataFrame) -> None:
    ...
    if metrics_all:
        ...
        save_summary_metric_plot(metrics_all_df, "pair_id", plots_dir / "pairwise_summary_metrics.png", "Pairwise RF | Accuracy and macro F1")
...
def main() -> int:
    ...
    read_log_path = OUT_ROOT / "rf_read_log.txt"
    ...
    counts_dt.to_csv(OUT_ROOT / "rf_counts_by_dataset_species_behavior.csv", index=False)
    log(f"[OK] CSV saved: {OUT_ROOT / 'rf_counts_by_dataset_species_behavior.csv'}")
    ...
    lookup_dt.to_csv(OUT_ROOT / "rf_analysis_dataset_lookup.csv", index=False)
    log(f"[OK] CSV saved: {OUT_ROOT / 'rf_analysis_dataset_lookup.csv'}")

    save_duplicate_report(all_dt, OUT_ROOT / "rf_duplicate_window_report.csv")
    ...
    if not unmatched.empty:
        unmatched[["species", "dataset_group", "dataset_id", "dataset_folder"]].drop_duplicates().to_csv(
            OUT_ROOT / "rf_unmatched_dataset_keys.csv", index=False
        )
        raise RuntimeError("Some rows could not be mapped to analysis_dataset. Check rf_unmatched_dataset_keys.csv")
