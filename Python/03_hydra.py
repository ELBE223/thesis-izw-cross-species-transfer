...
def validate_no_unmapped(df: pd.DataFrame) -> None:
    ...
    export_cols = [c for c in ["species", "dataset_group", "dataset_id", "dataset_folder", "top_folder", "source_file", "full_raw_path"] if c in bad.columns]
    bad[export_cols].drop_duplicates().to_csv(OUT_ROOT / "unmatched_dataset_keys.csv", index=False)
...
def main() -> int:
    try:
        ...
        if HydraClassifier is None:
            raise ImportError(
                "aeon is not available. Install aeon before running 03_hydra.py."
            ) from HYDRA_IMPORT_ERROR
