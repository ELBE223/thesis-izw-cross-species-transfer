...
OUT_ROOT = BASE / "Models" / "MultiRocket"
...
MULTIROCKET_N_JOBS = 1
MULTIROCKET_SETTINGS_NOTE = "aeon MultiRocketClassifier defaults; only random_state and n_jobs are fixed"
LOADING_SETTINGS_NOTE = "New final flat parquet loading: read direct per-dataset files from Data_Processed/data_processed_final"
CAP_SETTINGS_NOTE = "Training cap set to 250 per relevant group"
PREDICT_CHUNK_SIZE = 100
...
def valid_parquet_files(root: Path) -> list[Path]:
    ...
    The new Script in R 07 output should contain direct parquet files in
    Data_Processed/data_processed_final. Recursive loading is avoided so old
    nested part files cannot be mixed in accidentally.
    """
    if not root.exists():
        return []
    return sorted(
...
def add_analysis_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Use one capitalized behavior column expected by the MultiRocket code.
...
# Pipeline overview
# =========================================================
def _format_metric_summary(dt: pd.DataFrame) -> list[str]:
    metric_cols = ["accuracy", "macro_recall", "macro_precision", "macro_f1"]
    available = [col for col in metric_cols if col in dt.columns]
    if dt.empty or not available:
        return ["No metric rows available."]

    lines: list[str] = [f"Completed rows: {len(dt)}"]
    for col in available:
        values = pd.to_numeric(dt[col], errors="coerce").dropna()
        if values.empty:
            lines.append(f"{col}: n/a")
        else:
            lines.append(
                f"{col}: mean={values.mean():.4f}, min={values.min():.4f}, max={values.max():.4f}"
            )

    if "macro_f1" in dt.columns:
        tmp = dt.copy()
        tmp["macro_f1"] = pd.to_numeric(tmp["macro_f1"], errors="coerce")
        tmp = tmp.dropna(subset=["macro_f1"])
        if not tmp.empty:
            id_cols = [
                col for col in [
                    "analysis_dataset", "test_dataset", "pair_id",
                    "train_dataset", "family_id", "train_family", "test_family",
                ]
                if col in tmp.columns
            ]
            best = tmp.sort_values("macro_f1", ascending=False).iloc[0]
            worst = tmp.sort_values("macro_f1", ascending=True).iloc[0]
            best_id = " | ".join(f"{col}={best[col]}" for col in id_cols) if id_cols else "best"
            worst_id = " | ".join(f"{col}={worst[col]}" for col in id_cols) if id_cols else "worst"
            lines.append(f"Best macro_f1: {best['macro_f1']:.4f} ({best_id})")
            lines.append(f"Worst macro_f1: {worst['macro_f1']:.4f} ({worst_id})")

    return lines


def _read_metric_file(path: Path) -> Optional[pd.DataFrame]:
    if not path.exists():
        return None
    try:
...
def _append_metric_table(lines: list[str], title: str, csv_path: Path) -> None:
    lines.append("")
    lines.append("-" * 60)
    lines.append(title)
    lines.append(f"Source: {csv_path}")

    dt = _read_metric_file(csv_path)
    if dt is None:
        lines.append("Status: metric file not found.")
        return

    if "read_error" in dt.columns:
        lines.append(f"Status: could not read metric file: {dt['read_error'].iloc[0]}")
        return

    lines.extend(_format_metric_summary(dt))
    lines.append("")
    lines.append("Metrics table:")
    lines.extend(dt.to_string(index=False).splitlines())


def save_pipeline_overview(enabled_modes: list[tuple[str, Path, Callable[..., None]]]) -> None:
    overview_path = OUT_ROOT / "MultiRocket_overview.txt"
    lines: list[str] = [
        "MultiRocket overview",
        "=" * 60,
        f"Output root: {OUT_ROOT}",
        f"Raw root: {RAW_ROOT}",
        f"Expected dataset count: {EXPECTED_DATASET_COUNT}",
        f"Behaviors: {', '.join(KEEP_BEHAVIORS)}",
        f"Test fraction: {TEST_FRAC}",
        f"MultiRocket settings: {MULTIROCKET_SETTINGS_NOTE}",
        f"Loading settings: {LOADING_SETTINGS_NOTE}",
        f"Cap settings: {CAP_SETTINGS_NOTE}",
        f"MultiRocket n_jobs: {MULTIROCKET_N_JOBS}",
        f"Prediction chunk size: {PREDICT_CHUNK_SIZE}",
        "Enabled modes: " + ", ".join(name for name, _, _ in enabled_modes),
    ]

    counts_path = OUT_ROOT / "counts_by_dataset_species_behavior.csv"
    counts_dt = _read_metric_file(counts_path)
    lines.append("")
    lines.append("-" * 60)
    lines.append("Input overview")
    if counts_dt is None:
        lines.append("Counts file not found.")
    elif "read_error" in counts_dt.columns:
        lines.append(f"Counts file could not be read: {counts_dt['read_error'].iloc[0]}")
    else:
        lines.append(f"Counts rows: {len(counts_dt)}")
        if "analysis_dataset" in counts_dt.columns:
            lines.append(f"Datasets: {counts_dt['analysis_dataset'].nunique()}")
        if "N" in counts_dt.columns:
            lines.append(f"Total rows after filters: {int(pd.to_numeric(counts_dt['N'], errors='coerce').fillna(0).sum())}")
        lines.append("")
        lines.append("Rows by dataset/species/behavior:")
        lines.extend(counts_dt.to_string(index=False).splitlines())

    mode_metric_files = [
        ("Global_MultiRocket | metrics by analysis dataset", GLOBAL_ROOT / "statistics" / "metrics_by_analysis_dataset.csv"),
        ("Within_MultiRocket | all metrics", WITHIN_ROOT / "statistics" / "metrics_all.csv"),
        ("Inter_MultiRocket | all metrics", INTER_ROOT / "statistics" / "metrics_all.csv"),
        ("Cross_MultiRocket | all metrics", CROSS_ROOT / "statistics" / "metrics_all.csv"),
        ("Pairwise_MultiRocket | all metrics", PAIRWISE_ROOT / "statistics" / "metrics_all.csv"),
    ]
    for title, csv_path in mode_metric_files:
        _append_metric_table(lines, title, csv_path)

    lines.append("")
    lines.append("-" * 60)
    lines.append("Status / skip logs from mode summaries")
    summary_files = [
        GLOBAL_ROOT / "statistics" / "summary.txt",
        WITHIN_ROOT / "statistics" / "summary.txt",
        INTER_ROOT / "statistics" / "summary.txt",
        CROSS_ROOT / "statistics" / "summary.txt",
        PAIRWISE_ROOT / "statistics" / "summary.txt",
    ]
    for path in summary_files:
        lines.append("")
        lines.append(f"Summary file: {path}")
        if not path.exists():
            lines.append("Status: summary file not found.")
            continue
        try:
            ...
            if text:
                ...
                lines.append("Status: summary file empty.")
        except Exception as exc:
            lines.append(f"Status: could not read summary file: {exc}")

    save_text_summary(overview_path, lines)
...
def main() -> int:
    try:
        ...
        if MultiRocketClassifier is None:
            raise ImportError(
                "aeon is not available. Install aeon before running 06_multirocket.py."
            ) from MULTIROCKET_IMPORT_ERROR
        ...
        save_text_summary(
            ...
            [
                ...
                f"LOADING_SETTINGS_NOTE={LOADING_SETTINGS_NOTE}",
                ...
                f"CAP_SETTINGS_NOTE={CAP_SETTINGS_NOTE}",
                f"MULTIROCKET_N_JOBS={MULTIROCKET_N_JOBS}",
                f"PREDICT_CHUNK_SIZE={PREDICT_CHUNK_SIZE}",
            ],
        )
        ...
        save_pipeline_overview(enabled_modes)
