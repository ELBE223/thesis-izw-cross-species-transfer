...
def add_analysis_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Final 07 files use dataset_name. Keep this as the analysis dataset.
    ...
    # Use a single capitalized behavior column expected by the model code.
...
def fit_lgbm_model(train_df: pd.DataFrame, test_df: pd.DataFrame) -> dict[str, object]:
    ...
    # Standard setting: use LightGBM defaults.
    # objective is fixed because this is a multiclass classification task.
    # random_state, n_jobs, and verbosity are fixed for reproducibility, speed, and cleaner logs.
    ...
    model_settings = (
        ...
        f"boosting_type={model.boosting_type}; "
...
def run_global_lgbm(all_dt: pd.DataFrame) -> None:
    ...
    save_summary_metric_plot(global_metrics_by_dataset, "analysis_dataset", plots_dir / DATASET_METRICS_PLOT_NAME, "Global LGBM | Metrics by dataset")
...
def run_within_lgbm(all_dt: pd.DataFrame) -> None:
    ...
    if metrics_all:
        ...
        save_summary_metric_plot(metrics_all_df, "analysis_dataset", plots_dir / SUMMARY_METRICS_PLOT_NAME, "Within LGBM | Accuracy and macro F1")
...
def run_inter_lgbm(all_dt: pd.DataFrame) -> None:
    ...
    if metrics_all:
        ...
        behavior_all_df.to_csv(stats_dir / BEHAVIOR_METRICS_ALL_NAME, index=False)
        varimp_all_df.to_csv(stats_dir / VARIABLE_IMPORTANCE_ALL_NAME, index=False)
        save_summary_metric_plot(metrics_all_df, "pair_id", plots_dir / SUMMARY_METRICS_PLOT_NAME, "Inter LGBM | Accuracy and macro F1")
...
def run_cross_lgbm(all_dt: pd.DataFrame) -> None:
    ...
    if metrics_all:
        ...
        behavior_all_df.to_csv(stats_dir / BEHAVIOR_METRICS_ALL_NAME, index=False)
        varimp_all_df.to_csv(stats_dir / VARIABLE_IMPORTANCE_ALL_NAME, index=False)
        save_summary_metric_plot(metrics_all_df, "test_dataset", plots_dir / SUMMARY_METRICS_PLOT_NAME, "Cross LGBM | Accuracy and macro F1")
...
def run_pairwise_lgbm(all_dt: pd.DataFrame) -> None:
    ...
    if metrics_all:
        ...
        behavior_all_df.to_csv(stats_dir / BEHAVIOR_METRICS_ALL_NAME, index=False)
        varimp_all_df.to_csv(stats_dir / VARIABLE_IMPORTANCE_ALL_NAME, index=False)
        save_summary_metric_plot(metrics_all_df, "pair_id", plots_dir / SUMMARY_METRICS_PLOT_NAME, "Pairwise LGBM | Accuracy and macro F1")
...
def main() -> int:
    ...
    if not unmatched.empty:
        ...
        raise RuntimeError(f"Some rows could not be mapped to analysis_dataset. Check {ROOT_UNMATCHED_NAME}")
