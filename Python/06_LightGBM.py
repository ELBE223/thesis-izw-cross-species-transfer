...
ROOT_LGBM_OVERVIEW_NAME = "LGBM_overview.txt"
...
def save_lgbm_overview(all_dt: pd.DataFrame) -> None:
    out_file = OUT_ROOT / ROOT_LGBM_OVERVIEW_NAME

    lines: list[str] = [
        "LGBM overview",
        "===========",
        "This file is overwritten on every full LGBM pipeline run.",
    ...
    append_mode_overview(
        lines,
        "Global_LGBM metrics by dataset",
...
def add_analysis_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Use one capitalized behavior column expected by the model code.
