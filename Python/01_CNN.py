...
OVERVIEW_NAME = "CNN_overview.txt"
...
def add_analysis_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Use one capitalized behavior column expected by the CNN code.
...
def fit_cnn_model(
    ...
    # Standard loss and optimizer: no class weights.
...
def write_cnn_overview(
    ...
    lines.append("Metrics: zero_division=0; macro metrics averaged over all three behavior classes")
    ...
    add_metric_overview(
        lines,
        "Global_CNN overall metrics",
        GLOBAL_ROOT / "statistics" / "overall_metrics.csv",
        [],
    )
    add_metric_overview(
        lines,
        "Global_CNN metrics by dataset",
    ...
    add_metric_overview(
        lines,
        "Pairwise_CNN metrics",
        PAIRWISE_ROOT / "statistics" / "metrics_all.csv",
        ["pair_id", "train_dataset", "test_dataset"],
    )

    add_text_file_excerpt(lines, "Within_CNN skip notes", WITHIN_ROOT / "statistics" / "within_summary.txt")
    add_text_file_excerpt(lines, "Inter_CNN skip notes", INTER_ROOT / "statistics" / "inter_summary.txt")
    add_text_file_excerpt(lines, "Cross_CNN skip notes", CROSS_ROOT / "statistics" / "cross_summary.txt")
    add_text_file_excerpt(lines, "Pairwise_CNN skip notes", PAIRWISE_ROOT / "statistics" / "pairwise_summary.txt")
...
def main() -> int:
    try:
        ...
        write_cnn_overview(all_dt, x_cols, y_cols, z_cols, device, mode_failures)
