...
# - Compare paired within-dataset and cross-dataset Macro-F1 scores across
#   models using summary statistics and paired Wilcoxon tests.
# - Export publication-ready tables, plots, and text reports for H1.
#
# Hypothesis:
...
# ── 1. Setup ──────────────────────────────────────────────────────────────────
# - Load required packages and install pacman if needed.
...
# ── 2. User settings ──────────────────────────────────────────────────────────
# - Control optional p-value labels in the output plots.
show_p_values    <- FALSE
p_value_position <- "bottom_right"   # "top_left" or "bottom_right"


# ── 3. Paths ──────────────────────────────────────────────────────────────────
# - Define input folders and create all output folders.
...
# ── 4. Constants ──────────────────────────────────────────────────────────────
# - Set model order, comparison order, and plot styles.
...
# standard file `metrics_all.csv`. Model folders are now inside models_dir:
...
# ── 6. Helpers ────────────────────────────────────────────────────────────────
# - Define reusable functions for formatting, statistics, data handling, plots, and reports.

# ---- 6.1 Formatting ----------------------------------------------------------
# - Format numbers, p-values, and report sections consistently.
...
# ---- 6.2 Statistics ----------------------------------------------------------
# - Run paired tests and calculate paired summary metrics.
...
# - Read metric files and build paired within/cross datasets.
...
read_metric_file <- function(model, comparison, file, file_exists = NULL) {
  if (!file.exists(file)) {
...
# ---- 6.4 Plotting ------------------------------------------------------------
# - Build clean manuscript-style plots for H1.
...
# - Import all metric files and standardise dataset identifiers.
...
pair_check <- metrics_h1 %>%
  dplyr::filter(!is.na(dataset_key)) %>%
  dplyr::count(model, dataset_key, comparison, name = "n") %>%
  tidyr::pivot_wider(names_from = comparison, values_from = n, values_fill = 0)

# Be robust if only one comparison type is available in the current run.
...
pair_check <- pair_check %>%
  dplyr::mutate(is_paired = cross > 0 & within > 0) %>%
  dplyr::arrange(model, dataset_key)
...
# ── 8. Summary stats ──────────────────────────────────────────────────────────
# - Summarise H1 metrics by model and comparison type.
...
# - Test paired within/cross differences for Macro-F1 and accuracy.
...
# ── 10. Paired summaries ──────────────────────────────────────────────────────
# - Calculate paired descriptive statistics for reporting.
...
# ── 11. Plot data ─────────────────────────────────────────────────────────────
# - Prepare long and wide paired datasets for plotting.
...
# ── 12. Main tables ───────────────────────────────────────────────────────────
# - Assemble final H1 result tables for manuscript reporting.
...
# ── 13. Reporting focus ───────────────────────────────────────────────────────
# - Create compact result strings for direct manuscript interpretation.
...
# ── 14. Plots ─────────────────────────────────────────────────────────────────
# - Generate and save all H1 plots in black-and-white and colour.
...
# ── 15. Export CSV ────────────────────────────────────────────────────────────
# - Write all intermediate and final H1 tables as CSV files.
...
# ── 16. Export TXT ────────────────────────────────────────────────────────────
# - Write plain-text reports documenting outputs and checks.
...
# ── 17. Console output ────────────────────────────────────────────────────────
# - Print key results and output paths to the console.
