...
# ── Constants ─────────────────────────────────────────────────────────────────
model_levels      <- c("CNN", "InceptionTime", "HYDRA", "MultiRocket", "RF", "LGBM")
...
metric_paths <- tribble(
  ...
  "RF",            "within", file.path(base_dir, "RF",           "Within_RF",            "statistics", "within_metrics_all.csv"),
  "RF",            "cross",  file.path(base_dir, "RF",           "Cross_RF",             "statistics", "cross_metrics_all.csv"),
  "CNN",           "within", file.path(base_dir, "CNN",          "Within_CNN",           "statistics", "within_summary_metrics.csv"),
  "CNN",           "cross",  file.path(base_dir, "CNN",          "Cross_CNN",            "statistics", "cross_summary_metrics.csv"),
  "InceptionTime", "within", file.path(base_dir, "InceptionTime", "Within_InceptionTime", "statistics", "within_summary_metrics.csv"),
  "InceptionTime", "cross",  file.path(base_dir, "InceptionTime", "Cross_InceptionTime",  "statistics", "cross_summary_metrics.csv"),
  "HYDRA",         "within", file.path(base_dir, "HYDRA",        "Within_HYDRA",         "statistics", "within_summary_metrics.csv"),
  "HYDRA",         "cross",  file.path(base_dir, "HYDRA",        "Cross_HYDRA",          "statistics", "cross_summary_metrics.csv"),
  "LGBM",          "within", file.path(base_dir, "LGBM",         "Within_LGBM",          "statistics", "within_metrics_all.csv"),
  "LGBM",          "cross",  file.path(base_dir, "LGBM",         "Cross_LGBM",           "statistics", "cross_metrics_all.csv"),
  "MultiRocket",   "within", file.path(base_dir, "MultiRocket",  "Within_MultiRocket",   "statistics", "within_summary_metrics.csv"),
  "MultiRocket",   "cross",  file.path(base_dir, "MultiRocket",  "Cross_MultiRocket",    "statistics", "cross_summary_metrics.csv")
)
...
get_plot_style <- function(plot_style = c("bw", "color")) {
  ...
  list(
    fill_values = c("cross" = "grey80", "within" = "white"),
    point_alpha = 0.95,
    strip_fill = "grey95",
    label_fill = "white"
  )
...
# ── Paired-only descriptive summaries ─────────────────────────────────────────
...
# ── Paired-only plot data ─────────────────────────────────────────────────────
paired_plot_macro_f1 <- metrics_h1 %>%
  split(.$model) %>%
  imap_dfr(~ build_paired_long(.x, "macro_f1") %>% mutate(model = .y)) %>%
  mutate(model = factor(model, levels = model_levels))

paired_plot_accuracy <- metrics_h1 %>%
  split(.$model) %>%
  imap_dfr(~ build_paired_long(.x, "accuracy") %>% mutate(model = .y)) %>%
  mutate(model = factor(model, levels = model_levels))
...
paired_id_registry <- paired_plot_macro_f1 %>%
...
# ── Plots ─────────────────────────────────────────────────────────────────────
print_section("GENERATING COMBINED BOXPLOTS")
...
for (plot_style in plot_styles) {
  ...
  macro_file <- paste0("01_h1_macro_f1_boxplot_points_", style_suffix, ".png")
  acc_file   <- paste0("02_h1_accuracy_boxplot_points_", style_suffix, ".png")

  save_plot(p_macro_f1_box, file.path(plots_dir, macro_file))
  save_plot(p_accuracy_box, file.path(plots_dir, acc_file))
...
write_csv(plot_index, file.path(csv_dir, "12_h1_plot_index.csv"))
write_csv(reporting_focus, file.path(csv_dir, "13_h1_reporting_focus.csv"))
...
files_lines <- c(
  ...
  paste0("- ", c(
    ...
    "12_h1_plot_index.csv",
    "13_h1_reporting_focus.csv"
  ))
)
