# =============================================================================
# H3 analysis: trait-distance effect - final trait version
# =============================================================================
...
# - Load the final core and extended trait CSVs for the primary and sensitivity distances.
...
# H3 uses the final manually curated trait files.
# The script searches the current working directory and project H3 folders first
...
make_extended_traits_ordered <- function(dt) {
  dt %>%
    mutate(
      ...
      across(c(dominant_locomotor_mode, dominant_feeding_foraging_mode, body_configuration_type, activity_pattern, vertical_use_motion_scope), as.factor)
    )
}
...
# - species_traits_core_h3_final.csv = 7-trait primary H3 core set
# - species_traits_extended_sensitivity_full_final.csv = core + 4 sensitivity traits
must_exist(c(
...
bodymass_matches <- manual_traits_core_raw %>%
  transmute(
    species = clean_species(species),
    log_body_mass_g = as.numeric(log_body_mass_g),
    body_mass_g = exp(log_body_mass_g),
    provenance_note = paste0("Loaded directly from ", basename(manual_traits_core_file), ".")
  )

print_section("TRAIT INPUT SOURCE")
cat("Trait input source:", trait_input_source, "\n")
cat("Core trait file:", manual_traits_core_file, "\n")
cat("Extended trait file:", manual_traits_extended_file, "\n")

print_section("BODY MASS VALUES")
...
summary_lines <- c(
  "# ===================================================",
  "# Main Analysis Summary - H3 final core/extended traits",
  "# ===================================================",
  "",
  ...
  "",
  "# ----- Scope -----",
  "Primary distance = 7-trait core functional-biomechanical distance without postural compactness",
  "Sensitivity distance = full extended distance with the four additional sensitivity traits",
...
report_lines <- c(
  paste0("H3 final output folder: ", out_dir),
  paste0("Models input folder: ", models_dir),
  paste0("Trait folder: ", traits_dir),
  paste0("Created on: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S")),
  "",
  "This report contains the final H3 analysis with a 7-trait core distance as primary and the full core-plus-sensitivity trait set as extended robustness analysis.",
...
if (nrow(bodymass_matches) > 0) {
  report_lines <- append_section(report_lines, "BODY MASS VALUES", bodymass_matches)
}
...
write_txt_report(file.path(txt_dir, "01_h3_analysis_report_final.txt"), report_lines)
write_txt_report(file.path(txt_dir, "02_h3_summary_final.txt"), summary_lines)
