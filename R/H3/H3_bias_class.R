...
# - Use the final H3 core trait CSV with continuous log body mass
#   and separate locomotor / feeding-foraging traits.
...
# Keep old project only as fallback for supplementary class-distribution tables.
...
# H3 bias uses the final manually curated trait CSVs.
...
cat("Core final trait file detected: ", basename(manual_traits_core_file), "\n", sep = "")
cat("Extended final trait file detected: ", basename(manual_traits_extended_file), "\n", sep = "")
...
if (nrow(unmapped_train) > 0 || nrow(unmapped_test) > 0 ||
    ...
    nrow(missing_behavior_datasets) > 0) {
  stop("Coverage check failed. Fix dataset mapping, final H3 trait CSVs, or supplementary dataset tables first.")
}
...
csv_exports <- list(
  "01_behavior_totals.csv"                 = behavior_totals,
  "02_dataset_class_profiles.csv"          = dataset_profile_wide,
  "03_dataset_class_distribution_long.csv" = dataset_profiles,
  "04_dataset_pair_transfer_metrics.csv"   = dataset_pair_level,
  "05_dataset_pair_bias_metrics.csv"       = pair_bias,
  "06_spearman_bias_checks.csv"            = spearman_checks,
  "07_predictor_spearman_matrix.csv"       = predictor_correlations,
  "08_control_model_coefficients.csv"      = bind_rows(lm_overall_unadjusted, lm_overall_adjusted, lm_per_model),
  "09_trait_distance_beta_compare.csv"     = trait_beta_compare,
  "10_plot_index.csv"                      = plot_index
)
...
summary_lines <- c(
  ...
  paste0("Extended traits: ", basename(manual_traits_extended_file)),
...
report_lines <- c(
  ...
  "Core Gower distance uses species_traits_core_h3_final.csv with log_body_mass_g and split locomotor / feeding-foraging traits.",
