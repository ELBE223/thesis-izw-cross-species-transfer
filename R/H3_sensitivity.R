...
# - Use continuous log_bodymass_g instead of ordinal body_size_class for the
#   Gower distance sensitivity analysis.
...
# H3 sensitivity still needs the manually curated H3 trait files.
# Prefer the new project folder, but fall back to the previous project folder
# if the trait directory was not copied yet.
...
# Preferred new trait files from the revised H3/bodymass workflow.
# These keep the same manual trait structure, but replace body_size_class with
# continuous log_bodymass_g for the Gower distance.
...
match_elton_bodymass <- function(elton_file, study_species) {
  ...
  if (nrow(unmatched) > 0) {
    stop(
      ...
      "\nAdd a transparent fallback before running H3 sensitivity."
    )
  }
...
core_traits_file <- pick_existing_file(logbodymass_core_file_candidates)
extended_traits_file <- pick_existing_file(logbodymass_extended_file_candidates)
extended_available <- !is.na(extended_traits_file)
study_species_expected <- sort(unique(unname(dataset_to_species)))

if (!is.na(core_traits_file)) {
  trait_input_source <- "precomputed_log_bodymass_trait_file"
  core_traits_label <- basename(core_traits_file)
  manual_traits_core_raw <- read_trait_file(core_traits_file, required_core_trait_cols)
  cat("Core log-bodymass trait file detected: ", basename(core_traits_file), "\n", sep = "")
} else {
  trait_input_source <- "manual_traits_plus_elton_mamfuncdat"
  core_traits_label <- paste0(basename(manual_traits_core_file), " + ", basename(elton_file))
  cat("Core log-bodymass trait file not found. Reconstructing log_bodymass_g from manual traits plus EltonTraits.\n")
  must_exist(c(manual_traits_core_file, elton_file))
...
if (extended_available) {
  cat("Extended log-bodymass trait file detected: ", basename(extended_traits_file), "\n", sep = "")
} else {
  cat("Extended log-bodymass trait file not found. Core functional-biomechanical sensitivity will still run.\n")
}
...
summary_lines <- c(
  "# ===================================================",
  "# Sensitivity Analysis Summary - H3_sensitivity_log_bodymass.R",
  "# ===================================================",
  "",
  ...
  paste0("Extended traits:   ", ifelse(extended_available, basename(extended_traits_file), "not found / not used")),
