# =============================================================================
# H3 Bias Analysis
# Quick bias screen for H3
# Goal: test whether the H3 distance effect remains after controlling for
...
supp_behavior_file <- file.path(base_dir, "Supp_Material", "csv", "dataset_behavior_distribution.csv")
supp_overview_file <- file.path(base_dir, "Supp_Material", "csv", "dataset_overview.csv")

out_dir   <- file.path(base_dir, "H3_bias_class_shift")
...
model_files <- tribble(
  ~model,           ~pairwise_dir,
  "RF",            file.path(base_dir, "RF",          "Pairwise_RF",          "statistics"),
  "LGBM",          file.path(base_dir, "LGBM",        "Pairwise_LGBM",        "statistics"),
  "CNN",           file.path(base_dir, "CNN",         "Pairwise_CNN",         "statistics"),
  "ResNet",        file.path(base_dir, "ResNet",      "Pairwise_ResNet",      "statistics"),
  "HYDRA",         file.path(base_dir, "HYDRA",       "Pairwise_HYDRA",       "statistics"),
  "MultiRocket",   file.path(base_dir, "MultiRocket", "Pairwise_MultiRocket", "statistics")
)
...
# ---- 6.3 Trait distance and statistics ---------------------------------------
...
# ── 7. Input checks ───────────────────────────────────────────────────────────
must_exist(c(model_files$pairwise_file, manual_traits_core_file, supp_behavior_file, supp_overview_file))
...
pairwise <- pmap_dfr(model_files, read_pairwise_model) %>%
  mutate(
...
dataset_overview <- read_dataset_overview(supp_overview_file)
behavior_long <- read_behavior_distribution(supp_behavior_file, dataset_overview = dataset_overview)
...
bias_overview <- pair_bias %>%
  summarise(
    ...
    n_train_datasets = n_distinct(train_dataset),
    n_test_datasets = n_distinct(test_dataset),
...
coef_plot_dt <- bind_rows(
  ...
  mutate(
    term = recode(
      term,
      z_trait_distance_core = "Trait distance",
...
plot_index <- tibble(
  ...
  description = c(
    ...
    "Scaled adjusted coefficients for trait distance, class shift, and size gap"
  )
)
...
summary_lines <- c(
  ...
  paste0("Behavior distribution file: ", basename(supp_behavior_file)),
  paste0("Dataset overview file: ", basename(supp_overview_file)),
  ...
  ifelse(
    ...
    "Trait distance remains negative after adding class shift and size gap.",
    "Trait distance no longer remains negative after adding class shift and size gap."
  ),
  ifelse(
