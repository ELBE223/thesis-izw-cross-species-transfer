# =============================================================================
# H3 analysis: trait-distance effect
# =============================================================================
# Author : Lucas Beseler
# Date   : 2026-05-15
#
# Purpose:
# - Test whether cross-species transfer performance declines with increasing
#   functional-biomechanical trait distance between source and target species.
# - Quantify trait-based species distances using Gower distances and relate
#   them to pairwise cross-species transfer performance.
# - Export publication-ready tables, plots, and text reports for H3.
#
# Hypothesis:
...
out_dir       <- file.path(output_r_root, "H3")

# H3 still needs the manually curated trait files. Prefer the new project folder,
# but fall back to the previous project folder if the trait directory was not copied yet.
...
# Preferred new trait files exported by H3_elton_bodymass.R.
# If these files are present, H3 loads them directly.
# If they are absent, H3 builds the same log-body-mass trait tables from
# the original manual trait files plus MamFuncDat.txt.
logbodymass_core_file_candidates <- c(
  file.path(traits_dir, "elton_bodymass_check", "species_traits_core_logbodymass_gower.csv"),
  file.path(traits_dir, "species_traits_core_logbodymass_gower.csv"),
  file.path(traits_dir, "elton_bodymass_check", "species_traits_core_with_elton_bodymass.csv")
)

logbodymass_extended_file_candidates <- c(
  file.path(traits_dir, "elton_bodymass_check", "species_traits_extended_logbodymass_gower.csv"),
  file.path(traits_dir, "species_traits_extended_logbodymass_gower.csv"),
  file.path(traits_dir, "elton_bodymass_check", "species_traits_extended_with_elton_bodymass.csv")
)

# Optional QC files exported by H3_elton_bodymass.R.
# These are used only for transparent reporting; they do not change the analysis.
bodymass_match_file_candidates <- c(
  file.path(traits_dir, "elton_bodymass_check", "elton_bodymass_matches.csv"),
  file.path(traits_dir, "elton_bodymass_check", "bodymass_match_provenance.csv")
)

elton_file <- file.path(traits_dir, "MamFuncDat.txt")
...
# Model folders are now inside:
# /Volumes/Z Slim/11_05_2026_Data_Analysis/Models/<Model>/<Mode_Model>/statistics/
model_files <- tribble(
...
required_manual_core_trait_cols <- c(
  "species",
  "body_size_class",
...
required_manual_extended_trait_cols <- c(
  "species",
  "body_size_class",
  "dominant_locomotor_mode",
  "body_configuration_type",
...
model_files <- model_files %>%
  ...
  mutate(
    ...
    include_model = pairwise_exists & within_exists,
    missing_reason = case_when(
      include_model ~ "available",
      !pairwise_exists & !within_exists ~ "missing pairwise and within files",
      !pairwise_exists ~ "missing pairwise file",
      !within_exists ~ "missing within file",
      TRUE ~ "unknown"
    )
  )
...
if (nrow(missing_model_files) > 0) {
  message(
    "Skipping models with incomplete H3 inputs:\n",
    paste0(
      "- ", missing_model_files$model, ": ", missing_model_files$missing_reason,
      collapse = "\n"
    )
  )
}
...
if (nrow(model_files) == 0) {
  stop("No complete H3 model input pairs found. Need at least one model with both pairwise and within metrics files.")
}
...
build_bodymass_candidates <- function(study_species) {
  exact_candidates <- tibble(
  ...
  manual_fallbacks <- tribble(
    ~species, ~candidate, ~priority, ~match_source,
    "Canis lupus familiaris",  "Canis lupus",    2L, "manual_fallback_parent_species",
    "Equus ferus przewalskii", "Equus caballus", 2L, "manual_fallback_close_species"
  ) %>%
    filter(species %in% study_species)
  
  bind_rows(exact_candidates, manual_fallbacks) %>%
    mutate(
      species_clean = clean_species(species),
      candidate_clean = clean_species(candidate)
    )
}

match_elton_bodymass <- function(elton_file, study_species) {
  must_exist(elton_file)
  ...
  mass_col      <- pick_col(elton_raw, c("BodyMass-Value", "BodyMass.Value", "BodyMass_Value"))
  source_col    <- pick_col(elton_raw, c("BodyMass-Source", "BodyMass.Source", "BodyMass_Source"))
  speclevel_col <- pick_col(elton_raw, c("BodyMass-SpecLevel", "BodyMass.SpecLevel", "BodyMass_SpecLevel"))
  
  elton_body <- elton_raw %>%
    transmute(
      elton_id = .data[[id_col]],
      elton_scientific = clean_species(.data[[species_col]]),
      bodymass_g = parse_number(as.character(.data[[mass_col]])),
      bodymass_source = .data[[source_col]],
      bodymass_spec_level = .data[[speclevel_col]],
      log_bodymass_g = log(bodymass_g),
      log10_bodymass_g = log10(bodymass_g)
    ) %>%
    filter(
      !is.na(elton_scientific),
      !is.na(bodymass_g),
      bodymass_g > 0
    )
  
  matched_bodymass <- build_bodymass_candidates(study_species) %>%
    left_join(elton_body, by = c("candidate_clean" = "elton_scientific")) %>%
    arrange(species, is.na(bodymass_g), priority) %>%
    group_by(species) %>%
    slice(1) %>%
    ungroup() %>%
    mutate(
      match_status = case_when(
        !is.na(bodymass_g) & match_source == "exact" ~ "exact",
        !is.na(bodymass_g) & str_detect(match_source, "^manual_fallback") ~ match_source,
        TRUE ~ "not_found"
      )
    ) %>%
    select(
  ...
  unmatched <- matched_bodymass %>% filter(match_status == "not_found")
  if (nrow(unmatched) > 0) {
    stop(
      "Some study species have no valid EltonTraits body mass match:\n",
      paste(unmatched$species, collapse = "\n"),
      "\nAdd a transparent fallback before running H3."
    )
  }
...
load_or_reconstruct_bodymass_qc <- function(match_file_candidates, traits_core_raw, study_species) {
  match_file <- pick_existing_file(match_file_candidates)
  preferred_cols <- c(
    "species",
    "match_status",
    "matched_name",
  ...
  if (!is.na(match_file)) {
    qc <- read_csv(match_file, show_col_types = FALSE) %>%
      mutate(across(where(is.character), str_trim))
    
    if ("species" %in% names(qc)) {
      qc <- qc %>%
        mutate(species = clean_species(species)) %>%
        filter(species %in% study_species)
      
      if (nrow(qc) > 0) {
        return(qc %>% select(any_of(preferred_cols), everything()))
      }
    }
  }
  
  # Fallback for cases where precomputed Gower-ready trait files are present
  # but the separate Elton match table is unavailable. This keeps the report
  # informative and avoids printing an empty 0 x 0 tibble.
  traits_core_raw %>%
    transmute(
      species = clean_species(species),
      match_status = "precomputed_trait_file",
      matched_name = species,
      elton_id = NA_character_,
      bodymass_g = exp(as.numeric(log_bodymass_g)),
      log_bodymass_g = as.numeric(log_bodymass_g),
      log10_bodymass_g = as.numeric(log_bodymass_g) / log(10),
      bodymass_source = NA_character_,
      bodymass_spec_level = NA_character_,
      provenance_note = "Loaded from precomputed log-body-mass trait file; separate Elton match table was not found."
    ) %>%
    filter(species %in% study_species)
}

add_log_bodymass_to_traits <- function(traits_df, bodymass_matches, required_cols) {
  traits_df %>%
    mutate(species = clean_species(species)) %>%
    select(-any_of(c("body_size_class", "bodymass_g", "log10_bodymass_g"))) %>%
    left_join(
      bodymass_matches %>% select(species, log_bodymass_g),
      by = "species"
    ) %>%
...
make_extended_traits_ordered <- function(dt) {
  dt %>%
    mutate(
      ...
      postural_compactness_body_profile_reduction = ordered(postural_compactness_body_profile_reduction, levels = c("low", "moderate", "high", "very_high")),
      across(c(dominant_locomotor_mode, body_configuration_type, activity_pattern, vertical_use_motion_scope), as.factor)
    )
}
...
# Model files are checked above. Models with incomplete pairwise/within inputs are skipped.
# Trait files are still checked strictly below because H3 cannot compute distances without them.
print_section("H3 MODEL INPUTS")
...
core_logbodymass_file <- pick_existing_file(logbodymass_core_file_candidates)
extended_logbodymass_file <- pick_existing_file(logbodymass_extended_file_candidates)

if (!is.na(core_logbodymass_file) && !is.na(extended_logbodymass_file)) {
  trait_input_source <- "precomputed_log_bodymass_trait_files"
  
  manual_traits_core_raw <- read_trait_file(core_logbodymass_file, required_core_trait_cols)
  manual_traits_extended_raw <- read_trait_file(extended_logbodymass_file, required_extended_trait_cols)
  
  bodymass_matches <- load_or_reconstruct_bodymass_qc(
    bodymass_match_file_candidates,
    manual_traits_core_raw,
    study_species_expected
  )
} else {
  trait_input_source <- "manual_traits_plus_elton_mamfuncdat"
  ...
  manual_traits_core_base <- read_trait_file(manual_traits_core_file, required_manual_core_trait_cols)
  manual_traits_extended_base <- read_trait_file(manual_traits_extended_file, required_manual_extended_trait_cols)
  
  bodymass_matches <- match_elton_bodymass(elton_file, study_species_expected)
  
  manual_traits_core_raw <- add_log_bodymass_to_traits(
    manual_traits_core_base,
    bodymass_matches,
    required_core_trait_cols
  )
  
  manual_traits_extended_raw <- add_log_bodymass_to_traits(
    manual_traits_extended_base,
    bodymass_matches,
    required_extended_trait_cols
  )
}

print_section("TRAIT INPUT SOURCE")
cat("Trait input source:", trait_input_source, "\n")
if (!is.na(core_logbodymass_file)) cat("Core log-bodymass file:", core_logbodymass_file, "\n")
if (!is.na(extended_logbodymass_file)) cat("Extended log-bodymass file:", extended_logbodymass_file, "\n")
if (nrow(bodymass_matches) > 0) {
  print_section("ELTON BODY MASS MATCHES")
  print(bodymass_matches, n = Inf)
}
...
input_overview <- pairwise %>%
  ...
  left_join(within_ref %>% dplyr::count(model, name = "within_rows"), by = "model") %>%
...
missing_within_rows <- analysis_loss %>%
  ...
  dplyr::count(model, test_dataset, sort = TRUE) %>%
  left_join(missing_within_keys, by = c("model", "test_dataset"))

# ── 12. Replication checks and summaries ──────────────────────────────────────
duplicate_pair_ids <- pairwise %>%
  dplyr::count(model, pair_id, sort = TRUE) %>%
  filter(n > 1)

duplicate_dataset_pairs <- pairwise %>%
  dplyr::count(model, train_dataset, test_dataset, sort = TRUE) %>%
  filter(n > 1)

species_pair_reuse <- analysis_dt %>%
  dplyr::count(model, train_species, test_species, trait_distance_core, sort = TRUE) %>%
  filter(n > 1)
...
summary_lines <- c(
  "# ===================================================",
  "# Main Analysis Summary - H3 log-bodymass revised",
  "# ===================================================",
  "",
  paste0("Date: ", Sys.time()),
  paste0("Models included: ", paste(model_files$model, collapse = ", ")),
  ...
  paste0("Models skipped due to missing/incomplete inputs: ", ifelse(nrow(missing_model_files) == 0, "none", paste(missing_model_files$model, collapse = ", "))),
  "",
  "# ----- Scope -----",
  "Primary distance = core biomechanical trait distance with log_bodymass_g",
  "Sensitivity distance = extended trait distance with log_bodymass_g",
...
report_lines <- c(
  ...
  "",
  "This report contains the revised H3 analysis with core traits as primary and extended traits as sensitivity; body_size_class is replaced by log_bodymass_g.",
...
if (nrow(bodymass_matches) > 0) {
  report_lines <- append_section(report_lines, "ELTON BODY MASS MATCHES", bodymass_matches)
}
