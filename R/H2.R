...
# - Export publication-ready tables, plots, and text reports for H2.
#
# Hypothesis:
...
# - Install pacman only if missing.
if (!requireNamespace("pacman", quietly = TRUE)) {
  install.packages("pacman", repos = "https://cloud.r-project.org")
}

pacman::p_load(readr, dplyr, tidyr, ggplot2, purrr, tibble, grid, tidyselect)


# ── 2. User settings ──────────────────────────────────────────────────────────
# - Control plot annotations.
# - Keep plotting options reproducible.
show_p_values    <- TRUE
...
# - Create output folders for CSV, plots, and TXT files.
...
# ── 4. Constants ──────────────────────────────────────────────────────────────
# - Define fixed factor levels and required input columns.
# - Use explicit ordering for reproducible tables and plots.
...
# For each (model, comparison) we either load a pre-aggregated behaviour CSV
# (RF, LGBM) or scan the statistics folder recursively for per-dataset
# behaviour files. Model folders are now inside models_dir: /Volumes/Z Slim/11_05_2026_Data_Analysis/Models/<Model>.
...
# Small utilities for console headers, number/p-value formatting, post-hoc
# direction labels, and safe arithmetic.
...
# ---- 6.2 Data loading --------------------------------------------------------
# Pick the first existing column from a set of candidates; collect per-dataset
# behaviour files recursively when no aggregated file is provided; and the
# main loader used by pmap_dfr() over the behavior_paths registry.
...
# ---- 6.3 Statistics ----------------------------------------------------------
# Friedman's test and paired Wilcoxon post-hoc comparisons on blocks that
# have values for all behaviour levels.
...
# ---- 6.4 Summaries -----------------------------------------------------------
# Descriptive tables at the (model, behavior) level.
...
# ---- 6.5 Plotting ------------------------------------------------------------
# Shared style/theme, facet-wise p-value labels, and the ggsave wrapper.
...
# ── 7. Load data ──────────────────────────────────────────────────────────────
# - Read all registered behaviour-level metric files.
# - Standardize model, comparison, behaviour, and dataset identifiers.
...
# - Evaluate behaviour effects within cross-dataset results.
# - Use complete dataset blocks for paired statistical tests.
...
# - Match within- and cross-dataset results by model, dataset, and behaviour.
# - Quantify F1, recall, and precision transfer loss.
...
# ── 10. Tables ────────────────────────────────────────────────────────────────
# - Build manuscript-style summary and post-hoc tables.
# - Add formatted p-values and effect-size information.
...
# ── 11. Plots ─────────────────────────────────────────────────────────────────
# - Generate main and appendix figures in black-white and color versions.
# - Save all figures with publication-level resolution.
...
# ── 12. Export CSV ────────────────────────────────────────────────────────────
# - Export all intermediate and final analysis tables.
# - Keep file names numbered for traceable outputs.
...
# - Record generated files for reproducibility.
...
# ── 14. Console output ────────────────────────────────────────────────────────
# - Print key results and output locations.
# - Provide a quick console-level run summary.
